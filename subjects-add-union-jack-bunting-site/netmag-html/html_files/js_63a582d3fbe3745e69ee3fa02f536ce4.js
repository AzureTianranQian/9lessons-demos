      ...
      if(url.indexOf('/search') <= 0 && url.indexOf('/tag') <= 0) {
         ...
         jQuery.getJSON(new_url, function(data, status) {
            if(status == ajax_success) {
               ...
               updatePageHistory(new_url.replace(/(&)ajax=1/,''));
            }
         });
      }
   }
...
function createAjaxLinks(sort_option)
{
   jQuery(sort_option).click(function() {
      ...
      if(!p.hasClass('active')) {
         ...
         jQuery.getJSON(url, function(data, status) {
            if(status == ajax_success) {
               updateAjaxElements(data);
               updatePageHistory(url.replace(/(&)ajax=1/,''));
            }
         });
      }
      return false;
...
function updateAjaxElements(data)
{
   ...
   if(data['status']) {
      if(data['data']) {
         ...
         jQuery('#page-content .item-list').remove();
         jQuery('#page-content').append(d);
         jQuery('#page-content').fadeTo('slow', 1);
      }
      if(data['pager']) {
         pg = jQuery(data['pager']);
         if(jQuery('.ajax-pager').length > 0) {
            jQuery('.ajax-pager').remove();   
         } 
         jQuery('#page-content').append(pg);
         createAjaxPager(pg);
      }
      else {
         jQuery('.ajax-pager').remove();
...
jQuery(document).ready(function(){
   ...
   if(jQuery('#login-register-block').length > 0) {
      ...
      jQuery('#profile-links .last a').addClass(ajax_loader_class);

      if(!jQuery.cookie('FUTURE_USER') || jQuery.cookie('FUTURE_USER') > 0) {
         jQuery.getJSON('/future.php?cmd=profile-check', function(data, status) {
             if(status == ajax_success) {
                 if(data.username != 'anonymous') {
                     ...
                     if(!jQuery.cookie('FUTURE_VIEW_LISTINGS') || data.update) {
                        setDefaultViewOption(data.viewopt);
                     }
                 }
                 else {
                     jQuery('#profile-links a').each(function() {
                        $(this).click(function() {
                           profileBarDropDown($(this));
                           return false;
                        });
                     });
                 }
             }
             jQuery('#profile-links .last a').removeClass(ajax_loader_class);
         });
      }
      else {
         jQuery('#profile-links a').each(function() {
   ...
   if(jQuery('.ajax-pager').length > 0) {
      jQuery('.ajax-pager').each(function() {
         createAjaxPager(this);
      });
   }
   ...
   if(jQuery('#future-search-block-form').length > 0) {
      jQuery('#future-search-block-form').attr('method', 'post');
   }

});

// Function to drop down and return
function profileBarDropDown(thisButton) {
	if (thisButton.hasClass("dropped-down")) {
         jQuery('#login-register-block .content').slideUp('slow');
         thisButton.removeClass("dropped-down");
	} else {
         jQuery('#login-register-block .content').slideDown('slow');
         thisButton.addClass("dropped-down");
	}
}
...
{
   t = jQuery(pager);
   t.next('.item-list').hide();
   t.css('display', 'block');
   t.click(function(e) {
      t.addClass(ajax_loader_class);
      var txtCopy = t.text();
      t.text('Loading...');
      e.preventDefault();
      url = t.attr('href');
         ...
         if(status == ajax_success) {
            if(data['status']) {
               if(data['data']) {
                  d = jQuery(data['data']);
                  d.hide();
                  t.prev().append(innerShiv(d.html()));
                  d.fadeIn('slow');
               }
               if(data['pager']) {
                  p = jQuery(data['pager']);
            ...
            t.removeClass(ajax_loader_class);
            t.text(txtCopy);
         }
      });
   });
...
function createViewOptions()
{
   ...
   if(jQuery('.list-view-options').length > 0) {
      ...
      setDefaultViewOption('cookie', 'grid-view');
   }
}

// function to set the default view for listings (if other than grid)
function setDefaultViewOption(viewopt, defaultopt)
{
   ck_viewopt = jQuery.cookie('FUTURE_VIEW_LISTINGS');
   if(!ck_viewopt && defaultopt) {
      ck_viewopt = defaultopt;
   }

   if(viewopt != ck_viewopt) {
      if(viewopt == 'cookie') {
         viewopt = ck_viewopt;
      }
      ...
      jQuery('#page-content .article-listings.'+remove_viewopt).removeClass(remove_viewopt).addClass(viewopt);
...
function deactivateElement(evt) {
    document.activeElement = null;
}

if (!document.activeElement) {
   document.addEventListener("focus", activateElement,   true);
   document.addEventListener("blur",  deactivateElement, true);
}
...
{
   // hide all the labels. Rely on the placeholders
   jQuery('form#'+form_id+' label').not(':has(input)').hide();

   // Call the fallback placeholder generator
   jQuery('form#'+form_id+' [placeholder]').placeholder();
}
   ...
   if (typeof(window.history.pushState) != 'function') {
      ...
      if(tutorial_section > 0) {
         jQuery('.tag-cloud a, .article-tags a').each(function() {
            jQuery(this).attr('href', jQuery(this).attr('href').replace(/\?/, '#na=') );
         });
      }
   }
}
